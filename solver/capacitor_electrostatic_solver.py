...
class CapacitorElectrostaticSolver2D(ElectrostaticSolver2D, Capacitor):
    ...
    def _solve(self) -> None:
        ...
        # the capacitor plates, including the boundary nodes.
        for entity_tag in [
                CapacitorEntityTag.GROUND_PLATE_TAG,
                CapacitorEntityTag.VDD_PLATE_TAG
        ]:
            node_tags = self.get_nodes(tag=entity_tag, dim=self.dimension())
        ...
        # Fill in the electric field equations for the nodes within the
        # dielectric, including the boundary nodes.
        ...
        # Fill in Poisson's equation for the internal nodes within the
        # dielectric.
        dielectric_internal_node_tags = self.get_nodes(
            tag=CapacitorEntityTag.DIELECTRIC_TAG,
            dim=self.dimension(),
            node_type=GmshNodeType.INTERNAL)
        for tag in dielectric_internal_node_tags:
            x, y = node_coordinates[tag]
            ...
            # Iterate through all adjacent triangles.
            for tag_neighbor1, tag_neighbor2 in triangle_neighbors.get_neighbors(
                    tag):
                x_neighbor1, y_neighbor1 = node_coordinates[tag_neighbor1]
                x_neighbor2, y_neighbor2 = node_coordinates[tag_neighbor2]
