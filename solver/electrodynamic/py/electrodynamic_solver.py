...
class ElectrodynamicSolver2D(ElectrodynamicSolver, ElectromagneticSolver2D):
    ...
    def _solve(self) -> None:
        ...
        # Fill in the equations corresponding to Gauss's law for magnetism for
        # every node in the mesh.
        _, triangle_node_tags = self.get_faces()
        triangle_neighbors = NeighborLookup(triangle_node_tags)
        for tag, node_coordinates in node_tag_to_coordinates.items():
        ...
        # Fill in the equations for the nodes within the conductors. For the
        # boundary nodes, only fill in the voltage boundary conditions and
        # Ampere's law.
        boundary_tags = set()
        for conductor_tag in conductor_entity_tags:
            ...
            # Set the voltage boundary conditions for the boundary nodes. The
            # field equations at the interface are provided by the adjacent
            # insulator region.
            ...
            for tag in conductor_boundary_node_tags:
                ...
                b[gauss_law_electric_potential_equation_index] = (
                    self._get_ac_phasor(conductor_tag))
            boundary_tags.update(conductor_boundary_node_tags)
